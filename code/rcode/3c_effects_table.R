...
load(file.path(path_models, 'twfe_faculty.RDs'))
load(file.path(path_models, 'twfe_under.RDs'))
load(file.path(path_models, 'twfe_mas.RDs'))
load(file.path(path_models, 'twfe_doc.RDs'))
load(file.path(path_models, 'twfe_inbuilding.RDs'))
...
est_cs_faculty <- c("Faculty ATT", faculty_mod_twfe[["coefficients"]], cs_faculty[["overall.att"]])
se_cs_faculty <- c("", faculty_mod_twfe[["se"]], cs_faculty[["overall.se"]])

est_cs_undergrad <- c("Undergrad ATT", under_mod_twfe[["coefficients"]], cs_undergrad[["overall.att"]])
se_cs_undergrad <- c("",under_mod_twfe[["se"]] , cs_undergrad[["overall.se"]])

est_cs_master <- c("Master ATT", mas_mod_twfe[["coefficients"]], cs_master[["overall.att"]])
se_cs_master <- c("", mas_mod_twfe[["se"]], cs_master[["overall.se"]])

est_cs_doctor <- c("Doctor ATT", doc_mod_twfe[["coefficients"]], cs_doctor[["overall.att"]])
se_cs_doctor <- c("", doc_mod_twfe[["se"]], cs_doctor[["overall.se"]])

est_cs_inbuilding <- c("Inbuilding ATT", inbuilding_mod_twfe[["coefficients"]], cs_inbuilding[["overall.att"]])
se_cs_inbuilding <- c("", inbuilding_mod_twfe[["se"]], cs_inbuilding[["overall.se"]])
